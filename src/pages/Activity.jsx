  ...
  const getActivityDescription = (activity, currentUsername) => {
    ...
    switch (type) {
      ...
      case 'EXPENSE_CREATED':
        return `${actorName} added "${context.transaction?.description || 'an expense'}" ${context.group?.groupName ? `to ${context.group.groupName}` : ''
          }`;

      case 'EXPENSE_UPDATED':
        return `${actorName} updated "${context.transaction?.description || 'an expense'}" ${context.group?.groupName ? `in ${context.group.groupName}` : ''
          }`;

      case 'EXPENSE_DELETED':
        return `${actorName} deleted "${context.transaction?.description || 'an expense'}" ${context.group?.groupName ? `from ${context.group.groupName}` : ''
          }`;
      ...
      // Group activities
      case 'GROUP_CREATED':
        return `${actorName} created ${isCurrentUser ? 'a new group' : 'the group'
      ...
      default:
        return `${actorName} performed an activity ${context.group?.groupName ? `in ${context.group.groupName}` : ''
          }`;
    }
  };
          ...
          <div className="flex flex-wrap gap-2">
            {filterOptions.map((option) => (
              <button
                ...
                className={`px-4 py-2.5 rounded-xl font-medium transition-all duration-200 flex items-center gap-2 ${filterType === option.value
                    ? 'bg-gradient-to-r from-blue-500 to-blue-600 text-white shadow-lg'
                    : 'bg-white/80 text-gray-700 hover:bg-white/90 border border-gray-200/50'
                  }`}
              >
                <span>{option.label}</span>
                <span className={`text-xs px-2 py-1 rounded-full ${filterType === option.value
